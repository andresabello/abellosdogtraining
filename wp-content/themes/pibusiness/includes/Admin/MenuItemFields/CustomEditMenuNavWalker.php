...
class CustomEditMenuNavWalker extends Walker_Nav_Menu
{
    /**
    ...
    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes); ?>">
        ...
        <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
            <?php if ($item->type === 'custom' || empty($item->type)) : ?>
            ...
            if ($depth === 1) :
                $imageData = [
                    'value' => '',
                    'name' => 'menu_item_image[' . $item_id . ']',
                    'imageSrc' => '',
                ];
                $iconData = [
                    'value' => '',
                    'name' => 'menu_item_icon[' . $item_id . ']',
                    'imageSrc' => '',
                ];
                $imageAttachmentId = esc_attr($item->image);
                $imageAttachmentUrl = wp_get_attachment_image_url($imageAttachmentId, 'full');
                $imageData['imageSrc'] = esc_url($imageAttachmentUrl);
                $imageData['value'] = $item->image;
                $iconAttachmentId = $item->icon;
                $iconAttachmentUrl = wp_get_attachment_image_url($iconAttachmentId, 'full');
                $iconData['imageSrc'] = esc_url($iconAttachmentUrl);
                $iconData['value'] = $item->icon;
                ?>
                <p class="field-custom description description-wide">
                    <label for="edit-menu-item-image-<?php echo $item_id; ?>">
                        <?php _e('Image'); ?><br/>
                        <?= getTemplate('partials/upload-field.php', $imageData); ?>
                    </label>
                </p>
                ...
                <p class="field-custom description description-wide">
                    <label for="edit-menu-item-icon-<?php echo $item_id; ?>">
                        <?php _e('Icon'); ?><br/>
                        <?= getTemplate('partials/upload-field.php', $iconData); ?>
                    </label>
                </p>
            ...
            <p class="field-custom description description-wide">
                <label for="menu_item_icon_html-<?php echo $item_id; ?>">
                    <?php _e('Icon Html'); ?><br/>
                    <input type="text" id="menu_item_icon_html-<?php echo $item_id; ?>"
                           class="widefat edit-menu-item-attr-title"
                           name="menu_item_icon_html[<?php echo $item_id; ?>]"
                           value="<?php echo esc_attr($item->iconHtml); ?>"/>
                </label>
            </p>

            <?php if ($depth === 0) : ?>
                <p class="field-is-full-width description description-thin">
                    <input type="checkbox" id="edit-menu-item-is-full-width-<?php echo $item_id; ?>" value="_blank"
                           name="menu-item-full-width[<?php echo $item_id; ?>]"<?php checked($item->full_width,
                        '_blank'); ?> />
                    <label for="edit-menu-item-is-full-width-<?php echo $item_id; ?>">
                        <?php _e('Full width sub-menu?'); ?>
                    </label>
                    <br/>
                    <label for="edit-menu-item-column-size-<?php echo $item_id; ?>">
                        <?php _e('Menu item column size'); ?>
                    </label>
                    <select name="menu-item-column-size[<?php echo $item_id; ?>]"
                            id="edit-menu-item-column-size-<?php echo $item_id; ?>"
                            class="widefat edit-menu-item-attr-title">
                        <option value="3" <?php echo esc_attr($item->columnSize) === '3' ? 'selected' : null; ?>>
                            4 columns
                        </option>
                        <option value="4" <?php echo esc_attr($item->columnSize) === '4' ? 'selected' : null; ?>>
                            3 columns
                        </option>
                        <option value="6" <?php echo esc_attr($item->columnSize) === '6' ? 'selected' : null; ?>>
                            2 columns
                        </option>
                        <option value="12" <?php echo esc_attr($item->columnSize) === '12' ? 'selected' : null; ?>>
                            1 column
                        </option>
                        <option value="5" <?php echo esc_attr($item->columnSize) === '5' ? 'selected' : null; ?>>
                            5 columns
                        </option>
                        <option value="6" <?php echo esc_attr($item->columnSize) === '6' ? 'selected' : null; ?>>
                            6 columns
                        </option>
                        <option value="7" <?php echo esc_attr($item->columnSize) === '7' ? 'selected' : null; ?>>
                            7 columns
                        </option>
                        <option value="8" <?php echo esc_attr($item->columnSize) === '8' ? 'selected' : null; ?>>
                            8 columns
                        </option>
                    </select>
                </p><br/>
            ...
            <?php if ($depth >= 1) : ?>
                <p class="field-menu-flyout description description-thin">
                    <label for="edit-menu-item-flyout-<?php echo $item_id; ?>">
                        <?php _e('Flyout (Default is right)'); ?><br/>
                        <select name="menu-item-flyout[<?php echo $item_id; ?>]"
                                class="widefat code edit-menu-item-flyout">
                            <option value="right" <?php echo esc_attr($item->flyout) === 'right' ? 'selected' : null; ?>>
                                Right
                            </option>
                            <option value="left" <?php echo esc_attr($item->flyout) === 'left' ? 'selected' : null; ?>>
                                Left
                            </option>
                        </select>
                    </label>
